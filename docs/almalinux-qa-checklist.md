...
- [ ] SSL issue jobs reuse an existing `/etc/letsencrypt/live/<cert-name>` certificate before calling Certbot, scan duplicate Certbot lineages like `<cert-name>-0001` when the exact path is missing, and recover from exact-set rate-limit errors when any reusable certificate exists
...
- [ ] Laravel legacy asset URLs beginning with `/public/` serve from the real Laravel `public/` directory; stale sibling-domain `APP_URL`/`ASSET_URL` values are normalized to the linked domain and sibling-domain asset references are included in health checks
