...
- Deployment Doctor/Guardian now recognizes Laravel/Supervisor starts that fail because the process cwd was set to a missing `public` directory, corrects the deployment root back to the Laravel app root, and starts `php artisan serve` from the app root even for backend-only Laravel projects without a `public` folder.
