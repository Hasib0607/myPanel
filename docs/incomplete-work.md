...
- Account split added cPanel-style account records, WHM account create/detail/manage UI, package limits, assignment/unassignment, activity history, account-owned domain/mail/file APIs, account dashboard, separate `panel_session` / `account_session` cookies, account role guards, suspend/delete resource handling, account file-root isolation, sysagent home scaffolding, and split listener defaults: WHM/admin on `8453`, account/cPanel on `3138`.
