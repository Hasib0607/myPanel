...
- Wildcard subdomain SSL now uses safe Nginx config names such as `domain-wildcard.example.com` for HTTP/HTTPS vhost publishing and returns a clear DNS-01 requirement message instead of sending unsupported `*.example.com` names through webroot Certbot flows.
