...
- Backend-only Laravel deployments without `public/index.php` no longer publish a dead Nginx upstream to their idle Supervisor process; linked domains serve the account `public_html` fallback until the project gains a web entrypoint.
