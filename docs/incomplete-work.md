...
- Deployment Doctor/Guardian now treats public-route `degraded` probes and HTTP 502/503/504 Bad Gateway responses as real failures, rewrites the generated deployment Nginx vhost with stale `server_name` cleanup, queues a process restart, probes the public route again, and keeps the deployment failed when Nginx still cannot reach the upstream.
- Laravel root reconciliation no longer stops at a parent folder merely because it contains `artisan`; it performs a bounded recursive scan (excluding dependency/cache/build trees), treats `public/index.php` as a direct web-root marker, and a nested Laravel root containing that marker wins before Nginx is published. True backend-only Laravel deployments no longer publish a dead Nginx upstream or an empty/index-less `public_html` fallback, preventing both 502 and 403 false-success routes.
- Backend-only Laravel deployments without `public/index.php` are valid deployments: the Supervisor/worker-safe process can be marked `RUNNING/HEALTHY`, scheduled health watch can recover a previous failed state, and public Nginx routing is skipped unless an indexed `public_html` fallback exists.
- Deployment start now handles Supervisor's transient abnormal-termination plus immediate `STARTING` race by polling deployment health before failing the release; Deployment Doctor reports this as a restart/health-wait class instead of a dead start.
- Backend-only Laravel deployments that skip public web routing now actively retire stale Nginx `server_name` configs for the linked domain before reload, preventing old static `public_html` vhosts from continuing to show Nginx 403 after a deployment has been reclassified as worker-safe; retire failures are logged as warnings instead of failing the backend-only release.
- SSL issue jobs now check for an existing Let's Encrypt certificate before calling Certbot, scan duplicate Certbot lineages such as `<cert-name>-0001` when the exact `/live/<cert-name>` path is missing, reuse the found certificate to publish HTTPS/Nginx and sync database state, and recover from exact-set rate-limit errors instead of burning more duplicate certificate attempts.
...
- Deployment Doctor/Guardian now recognizes frontend build failures such as `vite: command not found` and `mix: command not found` as missing project package binaries, not missing server runtimes; Laravel frontend dependencies are installed with devDependencies before custom build commands, and redeploy repairs retry the build after reinstalling them even when Composer is the deployment package manager.
...
- Laravel deployment Nginx now safely maps legacy `/public/...` static asset URLs back to the actual Laravel `public/` document root, deployment env sync replaces stale sibling-domain public URLs such as `admin.example.com` when a project moves to `kiss.example.com`, and static-asset health checks include sibling-domain URLs instead of silently ignoring them.
- Deployment Doctor/Guardian now recognizes Composer lockfiles whose package constraints support PHP 8.1/8.2 but reject the current PHP 8.3 runtime, and queues the PHP 8.2 runtime switch instead of repeatedly treating the lockfile as a generic composer-update failure.
- Deployment Doctor/Guardian now treats deployments without a linked domain as valid internal-port deployments, skips Nginx proxy/SSL publishing instead of reading a null domain name, and recognizes the legacy `CONFIGURING_PROXY` null-domain failure as a redeployable panel-flow issue.
- The phased OpenAI-powered Deployment Doctor design is recorded in `docs/deployment-doctor-ai-plan.md`; it keeps AI diagnosis behind structured action catalogs, encrypted settings, approval gates, secret redaction, and sysagent command policy.
- Sysagent PHP 8.2/8.3 runtime repair now checks for the exact requested minor version instead of treating newer PHP as already repaired, synchronizes RHEL/AlmaLinux packages to the selected module stream, and verifies the CLI minor before Composer retries.
