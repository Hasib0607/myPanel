...
- Deployment Doctor/Guardian now recognizes Laravel/Supervisor starts that fail because `php artisan serve` needs a missing `public` directory, corrects accidental `/public` roots, and uses a long-running idle Supervisor process for backend-only Laravel projects without `public/index.php`.
