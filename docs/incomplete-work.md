...
- SSL issue jobs now check for an existing Let's Encrypt certificate before calling Certbot, reuse that certificate to publish HTTPS/Nginx and sync database state, and recover from exact-set rate-limit errors when a reusable cert already exists instead of burning more duplicate certificate attempts.
