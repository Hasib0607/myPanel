...
- Backend-only Laravel deployments that skip public web routing now actively retire stale Nginx `server_name` configs for the linked domain before reload, preventing old static `public_html` vhosts from continuing to show Nginx 403 after a deployment has been reclassified as worker-safe.
