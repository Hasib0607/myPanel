...
- Deployment Doctor/Guardian now recognizes Laravel/Supervisor starts that fail because `php artisan serve` needs a missing `public` directory, corrects accidental `/public` roots, detects nested Laravel app roots created by zip uploads such as `deployment/eBitans_Admin_Final/public/index.php`, uses the corrected app root for Nginx/start/health repair paths, uses a long-running idle Supervisor process for backend-only Laravel projects without `public/index.php`, skips `storage:link` for those backend-only projects, and skips public route probes so missing web roots do not create false danger-level 502 warnings.
