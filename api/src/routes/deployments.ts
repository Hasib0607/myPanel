...
function knownErrorHint(text: string): { message: string; repairAction: "set-node-memory" | "sync-public-env" | "sync-runtime" | "redeploy" | "restart" | "request-approval"; category: string } | null {
  ...
  if (lower.includes("ssl handshake failed") || lower.includes("invalid ssl response") || lower.includes("err_ssl_protocol")) return { message: "HTTPS is misconfigured on port 443 (broken nginx SSL listener). Redeploy after git pull so the panel can scrub stale configs and issue Let's Encrypt.", repairAction: "redeploy", category: "ssl_protocol_error" };
