...
if grep -qE '[[:space:]]client_max_body_size[[:space:]]+0[[:space:]]*;' "$NGINX_CONF"; then
  echo "Panel nginx already allows unlimited uploads"
  exit 0
fi

if awk '
  /location[[:space:]]+\/api\/v1\// { in_block = 1 }
  in_block && /client_max_body_size/ { found = 1 }
  in_block && /^[[:space:]]*}[[:space:]]*$/ { in_block = 0 }
  END { exit found ? 0 : 1 }
' "$NGINX_CONF"; then
  echo "Panel nginx API location already allows large uploads"
  exit 0
fi
...
echo "Patching $NGINX_CONF: enabling unlimited upload size for /api/v1/"
backup="${NGINX_CONF}.vps-panel.bak.$(date +%s)"
cp -a "$NGINX_CONF" "$backup"

sed -i -E '/^[[:space:]]*location[[:space:]]+\/api\/v1\//a\        client_max_body_size 0;' "$NGINX_CONF"
